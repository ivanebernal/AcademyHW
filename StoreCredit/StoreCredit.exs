textr = File.read! "C-large-practice.in"
text = String.split(textr, "\n")
cases = String.to_integer(Enum.fetch!(text,0))

File.rm("StoreCreditEX.txt")

credit = Enum.concat([0,0],text) |> Enum.take_every(3) |> Enum.drop(1) |> Enum.filter(fn x -> if x != "" do true end end) |> Enum.map(fn x -> String.to_integer(x) end)
...
for i <- 0..cases-1 do
	for j <- 0..Enum.fetch!(items, i)-2 do
		for k <- j+1..Enum.fetch!(items, i)-2 do
			...
			if (price1+price2) === Enum.fetch!(credit,i) do
